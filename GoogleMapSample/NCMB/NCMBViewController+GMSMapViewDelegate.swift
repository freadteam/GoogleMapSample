...
extension NCMBViewController: GMSMapViewDelegate {
    ...
    func mapView(_ mapView: GMSMapView, didLongPressAt coordinate: CLLocationCoordinate2D) {
        ...
        let okAction = UIAlertAction(title: "OK", style: .default) { (action) in
            ...
            let pin = Pin(latitude: coordinate.latitude, longitude: coordinate.longitude, title: (alert.textFields?.first?.text)!)
    ...
    func makeSaveButton() {
        let button = UIButton()
        button.frame = CGRect(x: 311, y: 460, width: 55, height: 55)
